...
const InStorageProcessedScreen = ({ navigate, appData }) => {
  ...
  const [qaReleased, setQaReleased] = React.useState(false);

  return (
    <PageWrapper title="In Storage" onBack={() => navigate('dashboard')}
      bottomBar={
        qaReleased
          ? <Btn label="Proceed to Dispatch →" onClick={() => navigate('dispatch', { palletState: 'QA_RELEASED' })} color="teal" />
          : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <Btn label="Awaiting QA Release…" disabled color="teal" />
              <button onClick={() => setQaReleased(true)} style={{
                background: 'none', border: 'none', color: C.muted,
                fontFamily: 'DM Sans,sans-serif', fontSize: 12, cursor: 'pointer',
              }}>Simulate QA release from Business Central →</button>
            </div>
          )
      }>
      <div style={{ padding: '20px 16px' }}>
        ...
        {!qaReleased && (
          <Alert type="info" message="QA review pending. This pallet cannot be dispatched until QA_RELEASED status is pushed from Business Central." />
        )}
        {qaReleased && (
          <Alert type="teal" message="✓ QA Released — pallet cleared for dispatch." />
        )}

        <Card style={{ padding: '14px 16px', marginBottom: 14 }}>
          <InfoRow label="Status" valueEl={<Badge state={qaReleased ? 'QA_RELEASED' : 'IN_STORAGE_PROCESSED'} label={qaReleased ? 'QA RELEASED' : 'PROCESSED'} />} />
          <InfoRow label="Zone" value="Zone C — Bay 02" />
...
const DispatchScreen = ({ navigate, appData }) => {
  ...
  return (
    <PageWrapper title="Dispatch" onBack={() => navigate('in_storage_processed')}
      bottomBar={
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
...
Object.assign(window, {
  InStorageProcessedScreen, QAHoldScreen, DispatchScreen,
